...
final class ModelData: ObservableObject {
    ...
    private init() {
        ...
        self.daf = Daf(name: "bogus", blatt: 0)
        self.myomi = "x"
        updateDateItems()
    ...
    public func updateDateItems() -> Void {
        ...
        if self.currentDay == today {
            ...
            do {
                try self.daf = Hebcal.dafYomi(date: now)
            } catch {
                self.daf = Daf(name: "bogus", blatt: 0)
            }
        }
    ...
    @discardableResult public func didTimezoneChange() -> Bool {
        if self.currentTimezone == TimeZone.current {
            ...
            logger.debug("timezone changed from \(self.currentTimezone.identifier) to \(TimeZone.current.identifier)")
